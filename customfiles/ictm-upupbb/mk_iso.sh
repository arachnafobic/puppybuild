...
elif [ $1 == "--without-uefi" ]; then
  echo "Turning UEFI off..."
  UEFI_ISO=
  rm -f ${BUILD}/boot/efi.img
fi
...
if [ $1 == "--uefi" ]; then
  OUT=${WOOF_OUTPUT}/${ISO_BASENAME}-uefi.iso
elif [ $1 == "--without-uefi" ]; then
...
(
	...
	if [ $1 == "--uefi" ]; then
		md5sum ${ISO_BASENAME}-uefi.iso > ${ISO_BASENAME}-uefi.iso.md5.txt
		sha256sum ${ISO_BASENAME}-uefi.iso > ${ISO_BASENAME}-uefi.iso.sha256.txt
	elif [ $1 == "--without-uefi" ]; then
